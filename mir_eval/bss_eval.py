...
'''
语音分离的评估指标计算模块。
本模块包含对SDR，SAR，SIR指标的计算。源代码修改自github项目：
...
def bss_eval_sources(reference_sources, estimated_sources,
    ...
    >>> # source
    >>> (sdr, sir, sar, perm) = bss_eval_sources(reference_sources, estimated_sources)
...
def bss_eval_sources_framewise(reference_sources, estimated_sources,
    ...
    >>> # source
    >>> (sdr, sir, sar,
    ...  perm) = bss_eval_sources_framewise(
             reference_sources,
    ...      estimated_sources)
...
def bss_eval_images(reference_sources, estimated_sources,
    ...
    >>> # estimated_sources[n] should be the same for the n'th estimated
    >>> # source
    >>> (sdr, isr, sir, sar, perm) = bss_eval_images(reference_sources, estimated_sources)
...
def bss_eval_images_framewise(reference_sources, estimated_sources,
    ...
    >>> # source
    >>> (sdr, isr, sir, sar,
    ...  perm) = bss_eval_images_framewise(
             reference_sources,
    ...      estimated_sources,
             window,
    ....     hop)
